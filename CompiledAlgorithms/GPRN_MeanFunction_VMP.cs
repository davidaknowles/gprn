using System;
using System.Linq;
using MicrosoftResearch.Infer;
using MicrosoftResearch.Infer.Distributions;
using MicrosoftResearch.Infer.Distributions.Kernels;
using MicrosoftResearch.Infer.Maths;
using gpnetworkVB;
using MicrosoftResearch.Infer.Collections;
using MicrosoftResearch.Infer.Factors;

namespace gpnetworkVB
{
	/// <summary>
	/// Class for performing inference in model 'GPRN' using algorithm 'VariationalMessagePassing'.
    /// This is the GPRN model with isotropic noise, node noise and a per output mean function.
	/// </summary>
	/// <remarks>
	/// The easiest way to use this class is to wrap an instance in a CompiledAlgorithm object and use
	/// the methods on CompiledAlgorithm to set parameters and execute inference.
	/// 
	/// If you instead wish to use this class directly, you must perform the following steps:
	/// 1) Create an instance of the class
	/// 2) Set the value of any externally-set fields e.g. data, priors
	/// 3) Call the Execute(numberOfIterations) method
	/// 4) Use the XXXMarginal() methods to retrieve posterior marginals for different variables.
	/// 
	/// Generated by Infer.NET 2.4 beta 3 at 13:41 on 07 November 2011.
	/// </remarks>
    [Serializable]
    public class GPRN_MeanFunction_VMP : IGeneratedAlgorithm, INetworkModel
	{
        public bool HasMeanFunctions() { return true; }

		#region Fields

        public KernelOptimiser nodeKernelOptimiser { get; set; }
        public KernelOptimiser weightKernelOptimiser { get; set; }

		/// <summary>Field backing the NumberOfIterationsDone property</summary>
		private int numberOfIterationsDone;
		/// <summary>Field backing the Q property</summary>
		private int q;
		/// <summary>Field backing the nodeNoisePrecisionPrior property</summary>
		private Gamma[] NodeNoisePrecisionPrior;
		/// <summary>Field backing the noisePrecisionPrior property</summary>
		private Gamma NoisePrecisionPrior;
		/// <summary>Field backing the D property</summary>
		private int d;
		/// <summary>Field backing the N property</summary>
		private int n;
		/// <summary>Field backing the vKernelFunction__0 property</summary>
		private KernelFunction[] MeanFunctionKernelFunctions;
		/// <summary>Field backing the K_weights_inverse property</summary>
		private PositiveDefiniteMatrix k_weights_inverse;
		/// <summary>Field backing the nodeFunctionsInitVar property</summary>
		private IDistribution<Vector[]> NodeFunctionsInitVar;
		/// <summary>Field backing the nodeSignalPrecisionsPrior property</summary>
		private Gamma[] NodeSignalPrecisionsPrior;
		/// <summary>Field backing the K_node_inverse property</summary>
		private PositiveDefiniteMatrix k_node_inverse;
		/// <summary>Field backing the observedData property</summary>
		private double[,] ObservedData;
		/// <summary>Field backing the isMissing property</summary>
		private bool[,] IsMissing;
		/// <summary>The number of iterations last computed by Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0. Set this to zero to force re-execution of Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0</summary>
		public int Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone;
		/// <summary>The number of iterations last computed by Constant. Set this to zero to force re-execution of Constant</summary>
		public int Constant_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2. Set this to zero to force re-execution of Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2</summary>
		public int Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_iterationsDone;
		/// <summary>True if Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2 has performed initialisation. Set this to false to force re-execution of Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2</summary>
		public bool Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised;
		/// <summary>The number of iterations last computed by Changed_D. Set this to zero to force re-execution of Changed_D</summary>
		public int Changed_D_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4. Set this to zero to force re-execution of Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4</summary>
		public int Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone;
		/// <summary>True if Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4 has performed initialisation. Set this to false to force re-execution of Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4</summary>
		public bool Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised;
		/// <summary>The number of iterations last computed by Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5. Set this to zero to force re-execution of Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5</summary>
		public int Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_iterationsDone;
		/// <summary>True if Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5 has performed initialisation. Set this to false to force re-execution of Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5</summary>
		public bool Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised;
		/// <summary>The number of iterations last computed by Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6. Set this to zero to force re-execution of Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6</summary>
		public int Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone;
		/// <summary>True if Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6 has performed initialisation. Set this to false to force re-execution of Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6</summary>
		public bool Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised;
		/// <summary>The number of iterations last computed by Changed_N. Set this to zero to force re-execution of Changed_N</summary>
		public int Changed_N_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_N_D. Set this to zero to force re-execution of Changed_N_D</summary>
		public int Changed_N_D_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_N_D_Q. Set this to zero to force re-execution of Changed_N_D_Q</summary>
		public int Changed_N_D_Q_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10. Set this to zero to force re-execution of Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10</summary>
		public int Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone;
		/// <summary>True if Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10 has performed initialisation. Set this to false to force re-execution of Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10</summary>
		public bool Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised;
		/// <summary>The number of iterations last computed by Changed_D_Q. Set this to zero to force re-execution of Changed_D_Q</summary>
		public int Changed_D_Q_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12. Set this to zero to force re-execution of Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12</summary>
		public int Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone;
		/// <summary>True if Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12 has performed initialisation. Set this to false to force re-execution of Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12</summary>
		public bool Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised;
		/// <summary>The number of iterations last computed by Changed_Q. Set this to zero to force re-execution of Changed_Q</summary>
		public int Changed_Q_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_Q_N. Set this to zero to force re-execution of Changed_Q_N</summary>
		public int Changed_Q_N_iterationsDone;
		/// <summary>The number of iterations last computed by Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15. Set this to zero to force re-execution of Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15</summary>
		public int Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone;
		/// <summary>True if Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15 has performed initialisation. Set this to false to force re-execution of Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15</summary>
		public bool Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised;
		/// <summary>The number of iterations last computed by Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16. Set this to zero to force re-execution of Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16</summary>
		public int Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone;
		/// <summary>True if Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16 has performed initialisation. Set this to false to force re-execution of Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16</summary>
		public bool Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised;
		public DistributionStructArray<Gaussian,double>[][] temp_B;
		/// <summary>Message from use of 'noisePrecision'</summary>
		public Gamma noisePrecision_use_B;
		/// <summary>Message to marginal of 'nodeFunctions'</summary>
		public DistributionRefArray<VectorGaussian,Vector> nodeFunctions_marginal_F;
		/// <summary>The constant 'vVector__5'</summary>
		public Vector[] xData;
		/// <summary>Message from use of 'meanFunctions'</summary>
		public DistributionRefArray<VectorGaussian,Vector> meanFunctions_use_B;
		/// <summary>Message from use of 'weightFunctions'</summary>
		public DistributionRefArray2D<VectorGaussian,Vector> weightFunctions_use_B;
		/// <summary>Message from use of 'nodeFunctionValues'</summary>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValues_use_B;
		/// <summary>Message to use of 'nodeFunctions'</summary>
		public DistributionRefArray<VectorGaussian,Vector> nodeFunctions_use_F;
		/// <summary>Message from use of 'nodeNoisePrecisions'</summary>
		public DistributionStructArray<Gamma,double> nodeNoisePrecisions_use_B;
		/// <summary>Buffer for GPFactor.AverageLogFactor</summary>
		public GPBuffer[] vint__0_Buffer;
		/// <summary>Message to marginal of 'ev'</summary>
		public Bernoulli ev_marginal_F;
		/// <summary>Message to marginal of 'nodeSignalPrecisions'</summary>
		public DistributionStructArray<Gamma,double> nodeSignalPrecisions_marginal_F;
		/// <summary>Message to marginal of 'nodeNoisePrecisions'</summary>
		public DistributionStructArray<Gamma,double> nodeNoisePrecisions_marginal_F;
		public DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]> weightFunctionValues_F;
		/// <summary>Message to marginal of 'nodeFunctionValues'</summary>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValues_marginal_F;
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> meanFunctionValues_F;
		public DistributionStructArray2D<Gaussian,double> noiseLessY_F;
		/// <summary>Message to marginal of 'noisePrecision'</summary>
		public Gamma noisePrecision_marginal_F;
		/// <summary>Message to marginal of 'nodeFunctionValuesPredictive'</summary>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValuesPredictive_marginal_F;
		public DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]> weightFunctionValuesPredictive_F;
		#endregion

		#region Properties
		/// <summary>The number of iterations done from the initial state</summary>
		public int NumberOfIterationsDone
		{			get {
				return this.numberOfIterationsDone;
			}
		}

		/// <summary>The externally-specified value of 'Q'</summary>
		public int Q
		{			get {
				return this.q;
			}
			set {
				if (this.q!=value) {
					this.q = value;
					this.numberOfIterationsDone = 0;
					this.Changed_N_D_Q_iterationsDone = 0;
					this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
					this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
					this.Changed_Q_iterationsDone = 0;
					this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone = 0;
					this.Changed_Q_N_iterationsDone = 0;
					this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 0;
					this.Changed_D_Q_iterationsDone = 0;
					this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone = 0;
					this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
					this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
					this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
					this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
				}
			}
		}

		/// <summary>The externally-specified value of 'nodeNoisePrecisionPrior'</summary>
		public Gamma[] nodeNoisePrecisionPrior
		{			get {
				return this.NodeNoisePrecisionPrior;
			}
			set {
				if ((value!=null)&&(value.Length!=this.q)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+this.q)+" was expected for variable \'nodeNoisePrecisionPrior\'");
				}
				this.NodeNoisePrecisionPrior = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone = 0;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'noisePrecisionPrior'</summary>
		public Gamma noisePrecisionPrior
		{			get {
				return this.NoisePrecisionPrior;
			}
			set {
				if (this.NoisePrecisionPrior!=value) {
					this.NoisePrecisionPrior = value;
					this.numberOfIterationsDone = 0;
					this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
					this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_iterationsDone = 0;
					this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
					this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
					this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
					this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
					this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
					this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
					this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
				}
			}
		}

		/// <summary>The externally-specified value of 'D'</summary>
		public int D
		{			get {
				return this.d;
			}
			set {
				if (this.d!=value) {
					this.d = value;
					this.numberOfIterationsDone = 0;
					this.Changed_N_D_iterationsDone = 0;
					this.Changed_N_D_Q_iterationsDone = 0;
					this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
					this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
					this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
					this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
					this.Changed_D_Q_iterationsDone = 0;
					this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone = 0;
					this.Changed_D_iterationsDone = 0;
					this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone = 0;
					this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone = 0;
					this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
					this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
				}
			}
		}

		/// <summary>The externally-specified value of 'N'</summary>
		public int N
		{			get {
				return this.n;
			}
			set {
				if (this.n!=value) {
					this.n = value;
					this.numberOfIterationsDone = 0;
					this.Changed_N_iterationsDone = 0;
					this.Changed_N_D_iterationsDone = 0;
					this.Changed_N_D_Q_iterationsDone = 0;
					this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
					this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
					this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
					this.Changed_Q_N_iterationsDone = 0;
					this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 0;
					this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
					this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
					this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
					this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
					this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
				}
			}
		}

		/// <summary>The externally-specified value of 'vKernelFunction__0'</summary>
		public KernelFunction[] vKernelFunction__0
		{			get {
				return this.MeanFunctionKernelFunctions;
			}
			set {
				if ((value!=null)&&(value.Length!=this.d)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+this.d)+" was expected for variable \'vKernelFunction__0\'");
				}
				this.MeanFunctionKernelFunctions = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone = 0;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'K_weights_inverse'</summary>
		public PositiveDefiniteMatrix K_weights_inverse
		{			get {
				return this.k_weights_inverse;
			}
			set {
				this.k_weights_inverse = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'nodeFunctionsInitVar'</summary>
		public IDistribution<Vector[]> nodeFunctionsInitVar
		{			get {
				return this.NodeFunctionsInitVar;
			}
			set {
				this.NodeFunctionsInitVar = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_iterationsDone = 0;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'nodeSignalPrecisionsPrior'</summary>
		public Gamma[] nodeSignalPrecisionsPrior
		{			get {
				return this.NodeSignalPrecisionsPrior;
			}
			set {
				if ((value!=null)&&(value.Length!=this.q)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+this.q)+" was expected for variable \'nodeSignalPrecisionsPrior\'");
				}
				this.NodeSignalPrecisionsPrior = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'K_node_inverse'</summary>
		public PositiveDefiniteMatrix K_node_inverse
		{			get {
				return this.k_node_inverse;
			}
			set {
				this.k_node_inverse = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'observedData'</summary>
		public double[,] observedData
		{			get {
				return this.ObservedData;
			}
			set {
				if ((value!=null)&&(((value.GetLength(0))!=this.d)||((value.GetLength(1))!=this.n))) {
					throw new ArgumentException(((("Provided array of size "+(((("("+(value.GetLength(0)))+",")+(value.GetLength(1)))+")"))+" when size ")+(((("("+this.d)+",")+this.n)+")"))+" was expected");
				}
				this.ObservedData = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		/// <summary>The externally-specified value of 'isMissing'</summary>
		public bool[,] isMissing
		{			get {
				return this.IsMissing;
			}
			set {
				if ((value!=null)&&(((value.GetLength(0))!=this.d)||((value.GetLength(1))!=this.n))) {
					throw new ArgumentException(((("Provided array of size "+(((("("+(value.GetLength(0)))+",")+(value.GetLength(1)))+")"))+" when size ")+(((("("+this.d)+",")+this.n)+")"))+" was expected");
				}
				this.IsMissing = value;
				this.numberOfIterationsDone = 0;
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
		}

		#endregion

		#region Methods
		/// <summary>Get the observed value of the specified variable.</summary>
		/// <param name="variableName">Variable name</param>
		public object GetObservedValue(string variableName)
		{
			if (variableName=="Q") {
				return this.Q;
			}
			if (variableName=="nodeNoisePrecisionPrior") {
				return this.nodeNoisePrecisionPrior;
			}
			if (variableName=="noisePrecisionPrior") {
				return this.noisePrecisionPrior;
			}
			if (variableName=="D") {
				return this.D;
			}
			if (variableName=="N") {
				return this.N;
			}
			if (variableName=="vKernelFunction__0") {
				return this.vKernelFunction__0;
			}
			if (variableName=="K_weights_inverse") {
				return this.K_weights_inverse;
			}
			if (variableName=="nodeFunctionsInitVar") {
				return this.nodeFunctionsInitVar;
			}
			if (variableName=="nodeSignalPrecisionsPrior") {
				return this.nodeSignalPrecisionsPrior;
			}
			if (variableName=="K_node_inverse") {
				return this.K_node_inverse;
			}
			if (variableName=="observedData") {
				return this.observedData;
			}
			if (variableName=="isMissing") {
				return this.isMissing;
			}
			throw new ArgumentException("Not an observed variable name: "+variableName);
		}

		/// <summary>Set the observed value of the specified variable.</summary>
		/// <param name="variableName">Variable name</param>
		/// <param name="value">Observed value</param>
		public void SetObservedValue(string variableName, object value)
		{
			if (variableName=="Q") {
				this.Q = (int)value;
				return ;
			}
			if (variableName=="nodeNoisePrecisionPrior") {
				this.nodeNoisePrecisionPrior = (Gamma[])value;
				return ;
			}
			if (variableName=="noisePrecisionPrior") {
				this.noisePrecisionPrior = (Gamma)value;
				return ;
			}
			if (variableName=="D") {
				this.D = (int)value;
				return ;
			}
            if (variableName == "xData")
            {
                this.xData = (Vector[])value;
                return; 
            }
			if (variableName=="N") {
				this.N = (int)value;
				return ;
			}
			if (variableName=="MeanFunctionKernelFunctions") {
				this.vKernelFunction__0 = (KernelFunction[])value;
				return ;
			}
			if (variableName=="K_weights_inverse") {
				this.K_weights_inverse = (PositiveDefiniteMatrix)value;
				return ;
			}
			if (variableName=="nodeFunctionsInitVar") {
				this.nodeFunctionsInitVar = (IDistribution<Vector[]>)value;
				return ;
			}
			if (variableName=="nodeSignalPrecisionsPrior") {
				this.nodeSignalPrecisionsPrior = (Gamma[])value;
				return ;
			}
			if (variableName=="K_node_inverse") {
				this.K_node_inverse = (PositiveDefiniteMatrix)value;
				return ;
			}
			if (variableName=="observedData") {
				this.observedData = (double[,])value;
				return ;
			}
			if (variableName=="isMissing") {
				this.isMissing = (bool[,])value;
				return ;
			}
			throw new ArgumentException("Not an observed variable name: "+variableName);
		}

		/// <summary>The marginal distribution of the specified variable.</summary>
		/// <param name="variableName">Variable name</param>
		public object Marginal(string variableName)
		{
			if (variableName=="ev") {
				return this.EvMarginal();
			}
			if (variableName=="nodeNoisePrecisions") {
				return this.NodeNoisePrecisionsMarginal();
			}
			if (variableName=="noisePrecision") {
				return this.NoisePrecisionMarginal();
			}
			if (variableName=="meanFunctionValues") {
				return this.MeanFunctionValuesMarginal();
			}
			if (variableName=="weightFunctionValues") {
				return this.WeightFunctionValuesMarginal();
			}
			if (variableName=="nodeSignalPrecisions") {
				return this.NodeSignalPrecisionsMarginal();
			}
			if (variableName=="nodeFunctionValues") {
				return this.NodeFunctionValuesMarginal();
			}
			if (variableName=="noiseLessY") {
				return this.NoiseLessYMarginal();
			}
			if (variableName=="nodeFunctionValuesPredictive") {
				return this.NodeFunctionValuesPredictiveMarginal();
			}
			if (variableName=="weightFunctionValuesPredictive") {
				return this.WeightFunctionValuesPredictiveMarginal();
			}
            if (variableName == "noisePrecisionArray")
            {
                return System.Linq.Enumerable.Range(0, D).Select(o => this.NoisePrecisionMarginal()).ToArray();
            }
			throw new ArgumentException("This class was not built to infer "+variableName);
		}

		public T Marginal<T>(string variableName)
		{
			return Distribution.ChangeType<T>(this.Marginal(variableName));
		}

		/// <summary>The query-specific marginal distribution of the specified variable.</summary>
		/// <param name="variableName">Variable name</param>
		/// <param name="query">QueryType name. For example, GibbsSampling answers 'Marginal', 'Samples', and 'Conditionals' queries</param>
		public object Marginal(string variableName, string query)
		{
			if (query=="Marginal") {
				return this.Marginal(variableName);
			}
			throw new ArgumentException(((("This class was not built to infer \'"+variableName)+"\' with query \'")+query)+"\'");
		}

		public T Marginal<T>(string variableName, string query)
		{
			return Distribution.ChangeType<T>(this.Marginal(variableName, query));
		}

		/// <summary>The output message of the specified variable.</summary>
		/// <param name="variableName">Variable name</param>
		public object GetOutputMessage(string variableName)
		{
			throw new ArgumentException("This class was not built to compute an output message for "+variableName);
		}

		/// <summary>Update all marginals, by iterating message passing the given number of times</summary>
		/// <param name="numberOfIterations">The number of times to iterate each loop</param>
		/// <param name="initialise">If true, messages that initialise loops are reset when observed values change</param>
		private void Execute(int numberOfIterations, bool initialise)
		{
			if (numberOfIterations<this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone) {
				this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = false;
				this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = false;
				this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = false;
				this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = false;
				this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = false;
				this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = false;
				this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = false;
				this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = false;
				this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			}
			this.Changed_N();
			this.Changed_N_D();
			this.Changed_N_D_Q();
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10(initialise);
			this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5(initialise);
			this.Constant();
			this.Changed_Q();
			this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16(initialise);
			this.Changed_Q_N();
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15(initialise);
			this.Changed_D_Q();
			this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12(initialise);
			this.Changed_D();
			this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6(initialise);
			this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4(initialise);
			this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2(initialise);
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0(numberOfIterations);
			this.numberOfIterationsDone = numberOfIterations;
		}

		public void Execute(int numberOfIterations)
		{
			this.Execute(numberOfIterations, true);
		}

		public void Update(int additionalIterations)
		{
			this.Execute(this.numberOfIterationsDone+additionalIterations, false);
		}

		private void OnProgressChanged(ProgressChangedEventArgs e)
		{
			// Make a temporary copy of the event to avoid a race condition
			// if the last subscriber unsubscribes immediately after the null check and before the event is raised.
			EventHandler<ProgressChangedEventArgs> handler = this.ProgressChanged;
			if (handler!=null) {
				handler(this, e);
			}
		}

		/// <summary>Reset all messages to their initial values.  Sets NumberOfIterationsDone to 0.</summary>
		public void Reset()
		{
			this.Execute(0);
		}

		/// <summary>Computations that depend on the observed value of N</summary>
		public void Changed_N()
		{
			if (this.Changed_N_iterationsDone==1) {
				return ;
			}
			// Create array for replicates of 'temp_B'
			this.temp_B = new DistributionStructArray<Gaussian,double>[this.n][];
			this.Changed_N_iterationsDone = 1;
			this.Changed_N_D_iterationsDone = 0;
			this.Changed_N_D_Q_iterationsDone = 0;
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of N and D</summary>
		public void Changed_N_D()
		{
			if (this.Changed_N_D_iterationsDone==1) {
				return ;
			}
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'temp_B'
				this.temp_B[n] = new DistributionStructArray<Gaussian,double>[this.d];
			}
			this.Changed_N_D_iterationsDone = 1;
			this.Changed_N_D_Q_iterationsDone = 0;
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of N and D and Q</summary>
		public void Changed_N_D_Q()
		{
			if (this.Changed_N_D_Q_iterationsDone==1) {
				return ;
			}
			for(int n = 0; n<N; n++) {
				for(int d = 0; d<D; d++) {
					// Create array for 'temp' Backwards messages.
					this.temp_B[n][d] = new DistributionStructArray<Gaussian,double>(this.q);
				}
			}
			this.Changed_N_D_Q_iterationsDone = 1;
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of N and D and Q and must reset on changes to numberOfIterationsDecreased and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10(bool initialise)
		{
			if ((this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone==1)&&((!initialise)||this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised)) {
				return ;
			}
			for(int n = 0; n<N; n++) {
				for(int d = 0; d<D; d++) {
					for(int q = 0; q<Q; q++) {
						this.temp_B[n][d][q] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
					}
				}
			}
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_iterationsDone = 1;
			this.Changed_N_D_Q_Init_numberOfIterationsDecreased_isMissing_observedData_noisePrecisionPrior_nodeNoiseP10_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of noisePrecisionPrior and must reset on changes to numberOfIterationsDecreased and Q and N and D and isMissing and observedData and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5(bool initialise)
		{
			if ((this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_iterationsDone==1)&&((!initialise)||this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised)) {
				return ;
			}
			this.noisePrecision_use_B = ArrayHelper.MakeUniform<Gamma>(this.NoisePrecisionPrior);
			this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_iterationsDone = 1;
			this.Changed_noisePrecisionPrior_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_nodeNoiseP5_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that do not depend on observed values</summary>
		public void Constant()
		{
			if (this.Constant_iterationsDone==1) {
				return ;
			}
			this.Constant_iterationsDone = 1;
			this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
			this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone = 0;
			this.Changed_D_iterationsDone = 0;
			this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone = 0;
			this.Changed_D_Q_iterationsDone = 0;
			this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone = 0;
			this.Changed_Q_iterationsDone = 0;
			this.Changed_Q_N_iterationsDone = 0;
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 0;
			this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of Q</summary>
		public void Changed_Q()
		{
			if (this.Changed_Q_iterationsDone==1) {
				return ;
			}
			// Create array for 'nodeFunctionValues_use' Backwards messages.
			this.nodeFunctionValues_use_B = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			// Create array for 'nodeNoisePrecisions_use' Backwards messages.
			this.nodeNoisePrecisions_use_B = new DistributionStructArray<Gamma,double>(this.q);
			this.Changed_Q_iterationsDone = 1;
			this.Changed_Q_N_iterationsDone = 0;
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 0;
			this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of Q and nodeNoisePrecisionPrior and must reset on changes to numberOfIterationsDecreased and N and D and isMissing and observedData and noisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16(bool initialise)
		{
			if ((this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone==1)&&((!initialise)||this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised)) {
				return ;
			}
			for(int q = 0; q<Q; q++) {
				this.nodeNoisePrecisions_use_B[q] = ArrayHelper.MakeUniform<Gamma>(this.NodeNoisePrecisionPrior[q]);
			}
			this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_iterationsDone = 1;
			this.Changed_Q_nodeNoisePrecisionPrior_Init_numberOfIterationsDecreased_N_D_isMissing_observedData_noiseP16_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of Q and N</summary>
		public void Changed_Q_N()
		{
			if (this.Changed_Q_N_iterationsDone==1) {
				return ;
			}
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValues_use' Backwards messages.
				this.nodeFunctionValues_use_B[q] = new DistributionStructArray<Gaussian,double>(this.n);
			}
			this.Changed_Q_N_iterationsDone = 1;
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of Q and N and must reset on changes to numberOfIterationsDecreased and D and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15(bool initialise)
		{
			if ((this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone==1)&&((!initialise)||this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised)) {
				return ;
			}
			for(int q = 0; q<Q; q++) {
				for(int n = 0; n<N; n++) {
					this.nodeFunctionValues_use_B[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_iterationsDone = 1;
			this.Changed_Q_N_Init_numberOfIterationsDecreased_D_isMissing_observedData_noisePrecisionPrior_nodeNoiseP15_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of D and Q</summary>
		public void Changed_D_Q()
		{
			if (this.Changed_D_Q_iterationsDone==1) {
				return ;
			}
			// Create array for 'weightFunctions_use' Backwards messages.
			this.weightFunctions_use_B = new DistributionRefArray2D<VectorGaussian,Vector>(this.d, this.q);
			this.Changed_D_Q_iterationsDone = 1;
			this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of D and Q and must reset on changes to numberOfIterationsDecreased and N and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12(bool initialise)
		{
			if ((this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone==1)&&((!initialise)||this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised)) {
				return ;
			}
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					this.weightFunctions_use_B[d, q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
				}
			}
			this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_iterationsDone = 1;
			this.Changed_D_Q_Init_numberOfIterationsDecreased_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP12_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of D</summary>
		public void Changed_D()
		{
			if (this.Changed_D_iterationsDone==1) {
				return ;
			}
			// Create array for replicates of 'vint__0_Buffer'
			this.vint__0_Buffer = new GPBuffer[this.d];
			// Create array for 'meanFunctions_use' Backwards messages.
			this.meanFunctions_use_B = new DistributionRefArray<VectorGaussian,Vector>(this.d);
			this.Changed_D_iterationsDone = 1;
			this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone = 0;
			this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone = 0;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of D and must reset on changes to numberOfIterationsDecreased and Q and N and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6(bool initialise)
		{
			if ((this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone==1)&&((!initialise)||this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised)) {
				return ;
			}
			for(int d = 0; d<D; d++) {
				this.meanFunctions_use_B[d] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
			}
			this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_iterationsDone = 1;
			this.Changed_D_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecisionPrior_nodeNoiseP6_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of D and vKernelFunction__0 and must reset on changes to numberOfIterationsDecreased and Q and N and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4(bool initialise)
		{
			if ((this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone==1)&&((!initialise)||this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised)) {
				return ;
			}
			for(int d = 0; d<D; d++) {
				// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
				this.vint__0_Buffer[d] = GPFactor.BufferInit(this.MeanFunctionKernelFunctions[d], this.xData);
			}
			this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_iterationsDone = 1;
			this.Changed_D_vKernelFunction__0_Init_numberOfIterationsDecreased_Q_N_isMissing_observedData_noisePrecis4_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of nodeFunctionsInitVar and must reset on changes to numberOfIterationsDecreased and Q and N and D and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="initialise">If true, reset messages that initialise loops</param>
		public void Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2(bool initialise)
		{
			if ((this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_iterationsDone==1)&&((!initialise)||this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised)) {
				return ;
			}
			// Create array for 'nodeFunctions_marginal' Forwards messages.
			this.nodeFunctions_marginal_F = ArrayHelper.MakeCopy<DistributionRefArray<VectorGaussian,Vector>>((DistributionRefArray<VectorGaussian,Vector>)this.NodeFunctionsInitVar);
			// Create array for 'nodeFunctions_use' Forwards messages.
			this.nodeFunctions_use_F = ArrayHelper.MakeCopy<DistributionRefArray<VectorGaussian,Vector>>((DistributionRefArray<VectorGaussian,Vector>)this.NodeFunctionsInitVar);
			this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_iterationsDone = 1;
			this.Changed_nodeFunctionsInitVar_Init_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrec2_isInitialised = true;
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = 0;
		}

		/// <summary>Computations that depend on the observed value of numberOfIterationsDecreased and Q and N and D and isMissing and observedData and noisePrecisionPrior and nodeNoisePrecisionPrior and K_node_inverse and nodeSignalPrecisionsPrior and nodeFunctionsInitVar and vKernelFunction__0 and K_weights_inverse</summary>
		/// <param name="numberOfIterations">The number of times to iterate each loop</param>
		public void Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0(int numberOfIterations)
		{
			if (this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone==numberOfIterations) {
				return ;
			}
			// The constant 'vBernoulli0'
			Bernoulli vBernoulli0 = Bernoulli.FromLogOdds(0);
			this.ev_marginal_F = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli0);
			// Buffer for VectorGaussianScaledPrecisionOp.SampleMean
			// Create array for replicates of 'nodeFunctions_B_q__SampleVariance'
			PositiveDefiniteMatrix[] nodeFunctions_B_q__SampleVariance = new PositiveDefiniteMatrix[this.q];
			for(int q = 0; q<Q; q++) {
				// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
				nodeFunctions_B_q__SampleVariance[q] = VectorGaussianScaledPrecisionOp.SampleVarianceInit(this.nodeFunctions_marginal_F[q]);
			}
			// Buffer for VectorGaussianScaledPrecisionOp.AverageLogFactor
			// Create array for replicates of 'nodeFunctions_B_q__SampleMean'
			Vector[] nodeFunctions_B_q__SampleMean = new Vector[this.q];
			for(int q = 0; q<Q; q++) {
				// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
				nodeFunctions_B_q__SampleMean[q] = VectorGaussianScaledPrecisionOp.SampleMeanInit(this.nodeFunctions_marginal_F[q]);
			}
			// Buffer for VectorGaussianScaledPrecisionOp.AverageLogFactor
			// Create array for replicates of 'nodeFunctions_B_q__SampleMean_ESamplePrecisionSample'
			double[] nodeFunctions_B_q__SampleMean_ESamplePrecisionSample = new double[this.q];
			// Message from use of 'nodeSignalPrecisions'
			DistributionStructArray<Gamma,double> nodeSignalPrecisions_use_B = default(DistributionStructArray<Gamma,double>);
			// Create array for 'nodeSignalPrecisions_use' Backwards messages.
			nodeSignalPrecisions_use_B = new DistributionStructArray<Gamma,double>(this.q);
			for(int q = 0; q<Q; q++) {
				nodeSignalPrecisions_use_B[q] = ArrayHelper.MakeUniform<Gamma>(this.NodeSignalPrecisionsPrior[q]);
			}
			// Create array for 'nodeSignalPrecisions_marginal' Forwards messages.
			this.nodeSignalPrecisions_marginal_F = new DistributionStructArray<Gamma,double>(this.q);
			for(int q = 0; q<Q; q++) {
				this.nodeSignalPrecisions_marginal_F[q] = ArrayHelper.MakeUniform<Gamma>(this.NodeSignalPrecisionsPrior[q]);
			}
			DistributionRefArray<VectorGaussian,Vector> nodeFunctions_F = default(DistributionRefArray<VectorGaussian,Vector>);
			// Create array for 'nodeFunctions' Forwards messages.
			nodeFunctions_F = new DistributionRefArray<VectorGaussian,Vector>(this.q);
			for(int q = 0; q<Q; q++) {
				nodeFunctions_F[q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
			}
			// Create array for 'nodeNoisePrecisions_marginal' Forwards messages.
			this.nodeNoisePrecisions_marginal_F = new DistributionStructArray<Gamma,double>(this.q);
			for(int q = 0; q<Q; q++) {
				this.nodeNoisePrecisions_marginal_F[q] = ArrayHelper.MakeUniform<Gamma>(this.NodeNoisePrecisionPrior[q]);
			}
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValues_F = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'nodeFunctionValues' Forwards messages.
			nodeFunctionValues_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValues' Forwards messages.
				nodeFunctionValues_F[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int n = 0; n<N; n++) {
					nodeFunctionValues_F[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValuesClean_F = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'nodeFunctionValuesClean' Forwards messages.
			nodeFunctionValuesClean_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValuesClean' Forwards messages.
				nodeFunctionValuesClean_F[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int _iv3 = 0; _iv3<N; _iv3++) {
					nodeFunctionValuesClean_F[q][_iv3] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			VectorGaussian _hoist2 = default(VectorGaussian);
			DistributionRefArray2D<VectorGaussian,Vector> weightFunctions_F = default(DistributionRefArray2D<VectorGaussian,Vector>);
			// Create array for 'weightFunctions' Forwards messages.
			weightFunctions_F = new DistributionRefArray2D<VectorGaussian,Vector>(this.d, this.q);
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					weightFunctions_F[d, q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
				}
			}
			for(int q = 0; q<Q; q++) {
				for(int d = 0; d<D; d++) {
					_hoist2 = ArrayHelper.CopyStorage<VectorGaussian>(weightFunctions_F[d, q]);
					q = this.q-1;
					d = this.d-1;
				}
			}
			_hoist2 = VectorGaussianScaledPrecisionOp.SampleAverageLogarithm(1, this.k_weights_inverse, _hoist2);
			// Create array for replicates of 'temp_F'
			DistributionStructArray<Gaussian,double>[][] temp_F = new DistributionStructArray<Gaussian,double>[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'temp_F'
				temp_F[n] = new DistributionStructArray<Gaussian,double>[this.d];
				for(int d = 0; d<D; d++) {
					// Create array for 'temp' Forwards messages.
					temp_F[n][d] = new DistributionStructArray<Gaussian,double>(this.q);
					for(int q = 0; q<Q; q++) {
						temp_F[n][d][q] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
					}
				}
			}
			// Create array for 'weightFunctionValues' Forwards messages.
			this.weightFunctionValues_F = new DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]>(this.d, this.q);
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					// Create array for 'weightFunctionValues' Forwards messages.
					this.weightFunctionValues_F[d, q] = new DistributionStructArray<Gaussian,double>(this.n);
					for(int _iv1 = 0; _iv1<N; _iv1++) {
						this.weightFunctionValues_F[d, q][_iv1] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
					}
				}
			}
			// Create array for 'nodeFunctionValues_marginal' Forwards messages.
			this.nodeFunctionValues_marginal_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValues_marginal' Forwards messages.
				this.nodeFunctionValues_marginal_F[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int n = 0; n<N; n++) {
					this.nodeFunctionValues_marginal_F[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			// The constant 'vint__0'
			int[] vint__0 = new int[1] {0};
			// Message to marginal of 'meanFunctions'
			DistributionRefArray<VectorGaussian,Vector> meanFunctions_marginal_F = default(DistributionRefArray<VectorGaussian,Vector>);
			// Create array for 'meanFunctions_marginal' Forwards messages.
			meanFunctions_marginal_F = new DistributionRefArray<VectorGaussian,Vector>(this.d);
			for(int d = 0; d<D; d++) {
				meanFunctions_marginal_F[d] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
			}
			// Buffer for GPFactor.SampleMean
			// Create array for replicates of 'meanFunctions_B_d__SampleVariance'
			PositiveDefiniteMatrix[] meanFunctions_B_d__SampleVariance = new PositiveDefiniteMatrix[this.d];
			for(int d = 0; d<D; d++) {
				// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
				meanFunctions_B_d__SampleVariance[d] = GPFactor.SampleVarianceInit(meanFunctions_marginal_F[d]);
			}
			// Buffer for GPFactor.AverageLogFactor
			// Create array for replicates of 'meanFunctions_B_d__SampleMean'
			Vector[] meanFunctions_B_d__SampleMean = new Vector[this.d];
			for(int d = 0; d<D; d++) {
				// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
				meanFunctions_B_d__SampleMean[d] = GPFactor.SampleMeanInit(meanFunctions_marginal_F[d]);
			}
			DistributionRefArray<VectorGaussian,Vector> meanFunctions_F = default(DistributionRefArray<VectorGaussian,Vector>);
			// Create array for 'meanFunctions' Forwards messages.
			meanFunctions_F = new DistributionRefArray<VectorGaussian,Vector>(this.d);
			for(int d = 0; d<D; d++) {
				meanFunctions_F[d] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
			}
			// Create array for replicates of 'vdouble41_F'
			Gaussian[][] vdouble41_F = new Gaussian[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'vdouble41_F'
				vdouble41_F[n] = new Gaussian[this.d];
				for(int d = 0; d<D; d++) {
					vdouble41_F[n][d] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Create array for 'meanFunctionValues' Forwards messages.
			this.meanFunctionValues_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.d);
			for(int d = 0; d<D; d++) {
				// Create array for 'meanFunctionValues' Forwards messages.
				this.meanFunctionValues_F[d] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int _iv0 = 0; _iv0<N; _iv0++) {
					this.meanFunctionValues_F[d][_iv0] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Create array for 'noiseLessY' Forwards messages.
			this.noiseLessY_F = new DistributionStructArray2D<Gaussian,double>(this.d, this.n);
			for(int d = 0; d<D; d++) {
				for(int n = 0; n<N; n++) {
					this.noiseLessY_F[d, n] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Create array for replicates of 'noisePrecision_cond_isMissing_d__n__1_B'
			Gamma[][] noisePrecision_cond_isMissing_d__n__1_B = new Gamma[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'noisePrecision_cond_isMissing_d__n__1_B'
				noisePrecision_cond_isMissing_d__n__1_B[n] = new Gamma[this.d];
				for(int d = 0; d<D; d++) {
					if (!this.IsMissing[d, n]) {
						noisePrecision_cond_isMissing_d__n__1_B[n][d] = ArrayHelper.MakeUniform<Gamma>(this.NoisePrecisionPrior);
					}
				}
			}
			// Create array for replicates of 'noisePrecision_rep0_rep0_B'
			DistributionStructArray<Gamma,double>[] noisePrecision_rep0_rep0_B = new DistributionStructArray<Gamma,double>[this.n];
			for(int n = 0; n<N; n++) {
				// Create array for 'noisePrecision_rep0_rep0' Backwards messages.
				noisePrecision_rep0_rep0_B[n] = new DistributionStructArray<Gamma,double>(this.d);
				for(int d = 0; d<D; d++) {
					noisePrecision_rep0_rep0_B[n][d] = ArrayHelper.MakeUniform<Gamma>(this.NoisePrecisionPrior);
				}
			}
			DistributionStructArray<Gamma,double> noisePrecision_rep0_B = default(DistributionStructArray<Gamma,double>);
			// Create array for 'noisePrecision_rep0' Backwards messages.
			noisePrecision_rep0_B = new DistributionStructArray<Gamma,double>(this.n);
			for(int n = 0; n<N; n++) {
				noisePrecision_rep0_B[n] = ArrayHelper.MakeUniform<Gamma>(this.NoisePrecisionPrior);
			}
			this.noisePrecision_marginal_F = ArrayHelper.MakeUniform<Gamma>(this.NoisePrecisionPrior);
			// Create array for replicates of 'noiseLessY_d_n_cond_isMissing_d__n__1_B'
			Gaussian[][] noiseLessY_d_n_cond_isMissing_d__n__1_B = new Gaussian[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'noiseLessY_d_n_cond_isMissing_d__n__1_B'
				noiseLessY_d_n_cond_isMissing_d__n__1_B[n] = new Gaussian[this.d];
				for(int d = 0; d<D; d++) {
					if (!this.IsMissing[d, n]) {
						noiseLessY_d_n_cond_isMissing_d__n__1_B[n][d] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
					}
				}
			}
			// Message from use of 'noiseLessY'
			DistributionStructArray2D<Gaussian,double> noiseLessY_use_B = default(DistributionStructArray2D<Gaussian,double>);
			// Create array for 'noiseLessY_use' Backwards messages.
			noiseLessY_use_B = new DistributionStructArray2D<Gaussian,double>(this.d, this.n);
			for(int d = 0; d<D; d++) {
				for(int n = 0; n<N; n++) {
					noiseLessY_use_B[d, n] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Message from use of 'meanFunctionValues'
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> meanFunctionValues_use_B = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'meanFunctionValues_use' Backwards messages.
			meanFunctionValues_use_B = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.d);
			for(int d = 0; d<D; d++) {
				// Create array for 'meanFunctionValues_use' Backwards messages.
				meanFunctionValues_use_B[d] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int _iv0 = 0; _iv0<N; _iv0++) {
					meanFunctionValues_use_B[d][_iv0] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Create array for replicates of 'vdouble41_B'
			Gaussian[][] vdouble41_B = new Gaussian[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for replicates of 'vdouble41_B'
				vdouble41_B[n] = new Gaussian[this.d];
				for(int d = 0; d<D; d++) {
					vdouble41_B[n][d] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Message from use of 'weightFunctionValues'
			DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]> weightFunctionValues_use_B = default(DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'weightFunctionValues_use' Backwards messages.
			weightFunctionValues_use_B = new DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]>(this.d, this.q);
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					// Create array for 'weightFunctionValues_use' Backwards messages.
					weightFunctionValues_use_B[d, q] = new DistributionStructArray<Gaussian,double>(this.n);
					for(int _iv1 = 0; _iv1<N; _iv1++) {
						weightFunctionValues_use_B[d, q][_iv1] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
					}
				}
			}
			// Messages from use of 'weightFunctions'
			DistributionRefArray2D<VectorGaussian,Vector>[] weightFunctions_use_uses_B = default(DistributionRefArray2D<VectorGaussian,Vector>[]);
			// Create array for 'weightFunctions_use_uses' Backwards messages.
			weightFunctions_use_uses_B = new DistributionRefArray2D<VectorGaussian,Vector>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'weightFunctions_use_uses' Backwards messages.
				weightFunctions_use_uses_B[_ind] = new DistributionRefArray2D<VectorGaussian,Vector>(this.d, this.q);
				for(int d = 0; d<D; d++) {
					for(int q = 0; q<Q; q++) {
						weightFunctions_use_uses_B[_ind][d, q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
					}
				}
			}
			// Message to marginal of 'weightFunctions'
			DistributionRefArray2D<VectorGaussian,Vector> weightFunctions_marginal_F = default(DistributionRefArray2D<VectorGaussian,Vector>);
			// Create array for 'weightFunctions_marginal' Forwards messages.
			weightFunctions_marginal_F = new DistributionRefArray2D<VectorGaussian,Vector>(this.d, this.q);
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					weightFunctions_marginal_F[d, q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
				}
			}
			// Create array for replicates of 'nodeFunctionValues_rep0_B'
			DistributionStructArray<Gaussian,double>[][] nodeFunctionValues_rep0_B = new DistributionStructArray<Gaussian,double>[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for replicates of 'nodeFunctionValues_rep0_B'
				nodeFunctionValues_rep0_B[q] = new DistributionStructArray<Gaussian,double>[this.n];
				for(int n = 0; n<N; n++) {
					// Create array for 'nodeFunctionValues_rep0' Backwards messages.
					nodeFunctionValues_rep0_B[q][n] = new DistributionStructArray<Gaussian,double>(this.d);
					for(int d = 0; d<D; d++) {
						nodeFunctionValues_rep0_B[q][n][d] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
					}
				}
			}
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>[] nodeFunctionValuesClean_uses_B = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>[]);
			// Create array for 'nodeFunctionValuesClean_uses' Backwards messages.
			nodeFunctionValuesClean_uses_B = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'nodeFunctionValuesClean_uses' Backwards messages.
				nodeFunctionValuesClean_uses_B[_ind] = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
				for(int q = 0; q<Q; q++) {
					// Create array for 'nodeFunctionValuesClean_uses' Backwards messages.
					nodeFunctionValuesClean_uses_B[_ind][q] = new DistributionStructArray<Gaussian,double>(this.n);
					for(int _iv3 = 0; _iv3<N; _iv3++) {
						nodeFunctionValuesClean_uses_B[_ind][q][_iv3] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
					}
				}
			}
			// Message from use of 'nodeFunctionValuesPredictive'
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValuesPredictive_use_B = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'nodeFunctionValuesPredictive_use' Backwards messages.
			nodeFunctionValuesPredictive_use_B = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValuesPredictive_use' Backwards messages.
				nodeFunctionValuesPredictive_use_B[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int n = 0; n<N; n++) {
					nodeFunctionValuesPredictive_use_B[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValuesPredictive_F = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'nodeFunctionValuesPredictive' Forwards messages.
			nodeFunctionValuesPredictive_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValuesPredictive' Forwards messages.
				nodeFunctionValuesPredictive_F[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int n = 0; n<N; n++) {
					nodeFunctionValuesPredictive_F[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			// Create array for 'nodeFunctionValuesPredictive_marginal' Forwards messages.
			this.nodeFunctionValuesPredictive_marginal_F = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValuesPredictive_marginal' Forwards messages.
				this.nodeFunctionValuesPredictive_marginal_F[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int n = 0; n<N; n++) {
					this.nodeFunctionValuesPredictive_marginal_F[q][n] = ArrayHelper.MakeUniform<Gaussian>(Gaussian.Uniform());
				}
			}
			DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> nodeFunctionValuesClean_B = default(DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>);
			// Create array for 'nodeFunctionValuesClean' Backwards messages.
			nodeFunctionValuesClean_B = new DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>(this.q);
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeFunctionValuesClean' Backwards messages.
				nodeFunctionValuesClean_B[q] = new DistributionStructArray<Gaussian,double>(this.n);
				for(int _iv3 = 0; _iv3<N; _iv3++) {
					nodeFunctionValuesClean_B[q][_iv3] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
				}
			}
			// Message from use of 'nodeFunctions'
			DistributionRefArray<VectorGaussian,Vector> nodeFunctions_use_B = default(DistributionRefArray<VectorGaussian,Vector>);
			// Create array for 'nodeFunctions_use' Backwards messages.
			nodeFunctions_use_B = new DistributionRefArray<VectorGaussian,Vector>(this.q);
			for(int q = 0; q<Q; q++) {
				nodeFunctions_use_B[q] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(N));
			}
			// Create array for replicates of 'nodeNoisePrecisions_rep0_B'
			DistributionStructArray<Gamma,double>[] nodeNoisePrecisions_rep0_B = new DistributionStructArray<Gamma,double>[this.q];
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeNoisePrecisions_rep0' Backwards messages.
				nodeNoisePrecisions_rep0_B[q] = new DistributionStructArray<Gamma,double>(this.n);
				for(int n = 0; n<N; n++) {
					nodeNoisePrecisions_rep0_B[q][n] = ArrayHelper.MakeUniform<Gamma>(this.NodeNoisePrecisionPrior[q]);
				}
			}
			// Messages from use of 'nodeNoisePrecisions'
			DistributionStructArray<Gamma,double>[] nodeNoisePrecisions_use_uses_B = default(DistributionStructArray<Gamma,double>[]);
			// Create array for 'nodeNoisePrecisions_use_uses' Backwards messages.
			nodeNoisePrecisions_use_uses_B = new DistributionStructArray<Gamma,double>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'nodeNoisePrecisions_use_uses' Backwards messages.
				nodeNoisePrecisions_use_uses_B[_ind] = new DistributionStructArray<Gamma,double>(this.q);
				for(int q = 0; q<Q; q++) {
					nodeNoisePrecisions_use_uses_B[_ind][q] = ArrayHelper.MakeUniform<Gamma>(this.NodeNoisePrecisionPrior[q]);
				}
			}
			// Create array for replicates of 'nodeNoisePrecisions_rep1_B'
			DistributionStructArray<Gamma,double>[] nodeNoisePrecisions_rep1_B = new DistributionStructArray<Gamma,double>[this.q];
			for(int q = 0; q<Q; q++) {
				// Create array for 'nodeNoisePrecisions_rep1' Backwards messages.
				nodeNoisePrecisions_rep1_B[q] = new DistributionStructArray<Gamma,double>(this.n);
				for(int n = 0; n<N; n++) {
					nodeNoisePrecisions_rep1_B[q][n] = ArrayHelper.MakeUniform<Gamma>(this.NodeNoisePrecisionPrior[q]);
				}
				// Message to 'nodeFunctionValuesClean' from ArrayFromVector factor
				nodeFunctionValuesClean_F[q] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(this.nodeFunctions_use_F[q], nodeFunctionValuesClean_F[q]);
				for(int d = 0; d<D; d++) {
					// Message to 'weightFunctions_marginal' from Variable factor
					weightFunctions_marginal_F[d, q] = VariableVmpOp.MarginalAverageLogarithm<VectorGaussian>(this.weightFunctions_use_B[d, q], _hoist2, weightFunctions_marginal_F[d, q]);
					// Message to 'weightFunctionValues' from ArrayFromVector factor
					this.weightFunctionValues_F[d, q] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(weightFunctions_marginal_F[d, q], this.weightFunctionValues_F[d, q]);
				}
			}
			// Message to 'noisePrecision_marginal' from Variable factor
			this.noisePrecision_marginal_F = VariableVmpOp.MarginalAverageLogarithm<Gamma>(this.noisePrecision_use_B, this.NoisePrecisionPrior, this.noisePrecision_marginal_F);
			for(int n = 0; n<N; n++) {
				for(int d = 0; d<D; d++) {
					if (!this.IsMissing[d, n]) {
						// Message to 'noiseLessY_d_n_cond_isMissing_d__n__1' from Gaussian factor
						noiseLessY_d_n_cond_isMissing_d__n__1_B[n][d] = GaussianOp.MeanAverageLogarithm(this.ObservedData[d, n], this.noisePrecision_marginal_F);
						// Message to 'noiseLessY_use' from Copy factor
						noiseLessY_use_B[d, n] = Factor.Copy<Gaussian>(noiseLessY_d_n_cond_isMissing_d__n__1_B[n][d]);
					}
				}
			}
			for(int d = 0; d<D; d++) {
				// Message to 'meanFunctions' from GP factor
				meanFunctions_F[d] = GPFactor.SampleAverageLogarithm(1, this.vint__0_Buffer[d], meanFunctions_F[d]);
				// Message to 'meanFunctions_marginal' from Variable factor
				meanFunctions_marginal_F[d] = VariableVmpOp.MarginalAverageLogarithm<VectorGaussian>(this.meanFunctions_use_B[d], meanFunctions_F[d], meanFunctions_marginal_F[d]);
				// Message to 'meanFunctionValues' from ArrayFromVector factor
				this.meanFunctionValues_F[d] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(meanFunctions_marginal_F[d], this.meanFunctionValues_F[d]);
				for(int n = 0; n<N; n++) {
					// Message to 'vdouble41' from Plus factor
					vdouble41_B[n][d] = DoublePlusVmpOp.AAverageLogarithm(noiseLessY_use_B[d, n], this.meanFunctionValues_F[d][n]);
				}
			}
			for(int q = 0; q<Q; q++) {
				// Message to 'nodeNoisePrecisions_marginal' from Variable factor
				this.nodeNoisePrecisions_marginal_F[q] = VariableVmpOp.MarginalAverageLogarithm<Gamma>(this.nodeNoisePrecisions_use_B[q], this.NodeNoisePrecisionPrior[q], this.nodeNoisePrecisions_marginal_F[q]);
				for(int n = 0; n<N; n++) {
					// Message to 'nodeFunctionValues' from Gaussian factor
					nodeFunctionValues_F[q][n] = GaussianOp.SampleAverageLogarithm(nodeFunctionValuesClean_F[q][n], this.nodeNoisePrecisions_marginal_F[q]);
					// Message to 'nodeFunctionValues_marginal' from Variable factor
					this.nodeFunctionValues_marginal_F[q][n] = VariableVmpOp.MarginalAverageLogarithm<Gaussian>(this.nodeFunctionValues_use_B[q][n], nodeFunctionValues_F[q][n], this.nodeFunctionValues_marginal_F[q][n]);
				}
			}
			for(int d = 0; d<D; d++) {
				// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
				meanFunctions_B_d__SampleVariance[d] = GPFactor.SampleVariance(meanFunctions_marginal_F[d], meanFunctions_B_d__SampleVariance[d]);
			}
			for(int iteration = this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone; iteration<numberOfIterations; iteration++) {
				for(int q = 0; q<Q; q++) {
					// Message to 'nodeFunctionValuesClean' from ArrayFromVector factor
					nodeFunctionValuesClean_F[q] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(this.nodeFunctions_use_F[q], nodeFunctionValuesClean_F[q]);
					for(int n = 0; n<N; n++) {
						// Message to 'nodeFunctionValuesPredictive' from Gaussian factor
						nodeFunctionValuesPredictive_F[q][n] = GaussianOp.SampleAverageLogarithm(nodeFunctionValuesClean_F[q][n], this.nodeNoisePrecisions_marginal_F[q]);
						// Message to 'nodeFunctionValuesPredictive_marginal' from Variable factor
						this.nodeFunctionValuesPredictive_marginal_F[q][n] = VariableVmpOp.MarginalAverageLogarithm<Gaussian>(nodeFunctionValuesPredictive_use_B[q][n], nodeFunctionValuesPredictive_F[q][n], this.nodeFunctionValuesPredictive_marginal_F[q][n]);
						// Message to 'nodeNoisePrecisions_rep1' from Gaussian factor
						nodeNoisePrecisions_rep1_B[q][n] = GaussianOp.PrecisionAverageLogarithm(this.nodeFunctionValuesPredictive_marginal_F[q][n], nodeFunctionValuesClean_F[q][n]);
					}
					// Message to 'nodeNoisePrecisions_use_uses' from Replicate factor
					nodeNoisePrecisions_use_uses_B[1][q] = ReplicateOp.DefAverageLogarithm<Gamma>(nodeNoisePrecisions_rep1_B[q], nodeNoisePrecisions_use_uses_B[1][q]);
					for(int n = 0; n<N; n++) {
						for(int d = 0; d<D; d++) {
							// Message to 'temp' from Product factor
							temp_F[n][d][q] = GaussianProductVmpOp.ProductAverageLogarithm(this.weightFunctionValues_F[d, q][n], this.nodeFunctionValues_marginal_F[q][n]);
						}
					}
				}
				for(int n = 0; n<N; n++) {
					for(int d = 0; d<D; d++) {
						// Message to 'vdouble41' from Sum factor
						vdouble41_F[n][d] = FastSumOp.SumAverageLogarithm(temp_F[n][d]);
						// Message to 'meanFunctionValues_use' from Plus factor
						meanFunctionValues_use_B[d][n] = DoublePlusVmpOp.BAverageLogarithm(noiseLessY_use_B[d, n], vdouble41_F[n][d]);
					}
				}
				for(int d = 0; d<D; d++) {
					// Message to 'meanFunctions_use' from ArrayFromVector factor
					this.meanFunctions_use_B[d] = ArrayFromVectorOp.VectorAverageLogarithm(meanFunctionValues_use_B[d], this.meanFunctions_use_B[d]);
					// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
					meanFunctions_B_d__SampleMean[d] = GPFactor.SampleMean(meanFunctions_marginal_F[d], meanFunctions_B_d__SampleVariance[d], meanFunctions_B_d__SampleMean[d]);
					// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
					this.vint__0_Buffer[d] = GPFactor.Buffer(vint__0, this.vint__0_Buffer[d], this.xData, meanFunctions_B_d__SampleMean[d], meanFunctions_B_d__SampleVariance[d], 1);
					// Message to 'meanFunctions' from GP factor
					meanFunctions_F[d] = GPFactor.SampleAverageLogarithm(1, this.vint__0_Buffer[d], meanFunctions_F[d]);
					// Message to 'meanFunctions_marginal' from Variable factor
					meanFunctions_marginal_F[d] = VariableVmpOp.MarginalAverageLogarithm<VectorGaussian>(this.meanFunctions_use_B[d], meanFunctions_F[d], meanFunctions_marginal_F[d]);
					// Message to 'meanFunctionValues' from ArrayFromVector factor
					this.meanFunctionValues_F[d] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(meanFunctions_marginal_F[d], this.meanFunctionValues_F[d]);
					for(int n = 0; n<N; n++) {
						// Message to 'noiseLessY' from Plus factor
						this.noiseLessY_F[d, n] = DoublePlusVmpOp.SumAverageLogarithm(vdouble41_F[n][d], this.meanFunctionValues_F[d][n]);
						if (!this.IsMissing[d, n]) {
							// Message to 'noisePrecision_cond_isMissing_d__n__1' from Gaussian factor
							noisePrecision_cond_isMissing_d__n__1_B[n][d] = GaussianOp.PrecisionAverageLogarithm(this.ObservedData[d, n], this.noiseLessY_F[d, n]);
							// Message to 'noisePrecision_rep0_rep0' from Copy factor
							noisePrecision_rep0_rep0_B[n][d] = Factor.Copy<Gamma>(noisePrecision_cond_isMissing_d__n__1_B[n][d]);
						}
					}
				}
				for(int n = 0; n<N; n++) {
					// Message to 'noisePrecision_rep0' from Replicate factor
					noisePrecision_rep0_B[n] = ReplicateOp.DefAverageLogarithm<Gamma>(noisePrecision_rep0_rep0_B[n], noisePrecision_rep0_B[n]);
				}
				// Message to 'noisePrecision_use' from Replicate factor
				this.noisePrecision_use_B = ReplicateOp.DefAverageLogarithm<Gamma>(noisePrecision_rep0_B, this.noisePrecision_use_B);
				// Message to 'noisePrecision_marginal' from Variable factor
				this.noisePrecision_marginal_F = VariableVmpOp.MarginalAverageLogarithm<Gamma>(this.noisePrecision_use_B, this.NoisePrecisionPrior, this.noisePrecision_marginal_F);
				for(int n = 0; n<N; n++) {
					for(int d = 0; d<D; d++) {
						if (!this.IsMissing[d, n]) {
							// Message to 'noiseLessY_d_n_cond_isMissing_d__n__1' from Gaussian factor
							noiseLessY_d_n_cond_isMissing_d__n__1_B[n][d] = GaussianOp.MeanAverageLogarithm(this.ObservedData[d, n], this.noisePrecision_marginal_F);
							// Message to 'noiseLessY_use' from Copy factor
							noiseLessY_use_B[d, n] = Factor.Copy<Gaussian>(noiseLessY_d_n_cond_isMissing_d__n__1_B[n][d]);
						}
						// Message to 'vdouble41' from Plus factor
						vdouble41_B[n][d] = DoublePlusVmpOp.AAverageLogarithm(noiseLessY_use_B[d, n], this.meanFunctionValues_F[d][n]);
						// Message to 'temp' from Sum factor
						this.temp_B[n][d] = FastSumOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(vdouble41_B[n][d], temp_F[n][d], this.temp_B[n][d]);
						for(int q = 0; q<Q; q++) {
							// Message to 'weightFunctionValues_use' from Product factor
							weightFunctionValues_use_B[d, q][n] = GaussianProductVmpOp.AAverageLogarithm(this.temp_B[n][d][q], this.nodeFunctionValues_marginal_F[q][n]);
						}
					}
				}
				for(int q = 0; q<Q; q++) {
					for(int d = 0; d<D; d++) {
						// Message to 'weightFunctions_use_uses' from ArrayFromVector factor
						weightFunctions_use_uses_B[0][d, q] = ArrayFromVectorOp.VectorAverageLogarithm(weightFunctionValues_use_B[d, q], weightFunctions_use_uses_B[0][d, q]);
					}
				}
				// Message to 'weightFunctions_use' from Replicate factor
				this.weightFunctions_use_B = ReplicateOp.DefAverageLogarithm<DistributionRefArray2D<VectorGaussian,Vector>>(weightFunctions_use_uses_B, this.weightFunctions_use_B);
				for(int q = 0; q<Q; q++) {
					for(int d = 0; d<D; d++) {
						// Message to 'weightFunctions_marginal' from Variable factor
						weightFunctions_marginal_F[d, q] = VariableVmpOp.MarginalAverageLogarithm<VectorGaussian>(this.weightFunctions_use_B[d, q], _hoist2, weightFunctions_marginal_F[d, q]);
						// Message to 'weightFunctionValues' from ArrayFromVector factor
						this.weightFunctionValues_F[d, q] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(weightFunctions_marginal_F[d, q], this.weightFunctionValues_F[d, q]);
						for(int n = 0; n<N; n++) {
							// Message to 'temp' from Product factor
							temp_F[n][d][q] = GaussianProductVmpOp.ProductAverageLogarithm(this.weightFunctionValues_F[d, q][n], this.nodeFunctionValues_marginal_F[q][n]);
						}
					}
				}
				for(int n = 0; n<N; n++) {
					for(int d = 0; d<D; d++) {
						// Message to 'temp' from Sum factor
						this.temp_B[n][d] = FastSumOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(vdouble41_B[n][d], temp_F[n][d], this.temp_B[n][d]);
						for(int q = 0; q<Q; q++) {
							// Message to 'nodeFunctionValues_rep0' from Product factor
							nodeFunctionValues_rep0_B[q][n][d] = GaussianProductVmpOp.BAverageLogarithm(this.temp_B[n][d][q], this.weightFunctionValues_F[d, q][n]);
						}
					}
					for(int q = 0; q<Q; q++) {
						// Message to 'nodeFunctionValues_use' from Replicate factor
						this.nodeFunctionValues_use_B[q][n] = ReplicateOp.DefAverageLogarithm<Gaussian>(nodeFunctionValues_rep0_B[q][n], this.nodeFunctionValues_use_B[q][n]);
						// Message to 'nodeFunctionValues' from Gaussian factor
						nodeFunctionValues_F[q][n] = GaussianOp.SampleAverageLogarithm(nodeFunctionValuesClean_F[q][n], this.nodeNoisePrecisions_marginal_F[q]);
						// Message to 'nodeFunctionValues_marginal' from Variable factor
						this.nodeFunctionValues_marginal_F[q][n] = VariableVmpOp.MarginalAverageLogarithm<Gaussian>(this.nodeFunctionValues_use_B[q][n], nodeFunctionValues_F[q][n], this.nodeFunctionValues_marginal_F[q][n]);
						// Message to 'nodeNoisePrecisions_rep0' from Gaussian factor
						nodeNoisePrecisions_rep0_B[q][n] = GaussianOp.PrecisionAverageLogarithm(this.nodeFunctionValues_marginal_F[q][n], nodeFunctionValuesClean_F[q][n]);
					}
				}
				for(int q = 0; q<Q; q++) {
					// Message to 'nodeNoisePrecisions_use_uses' from Replicate factor
					nodeNoisePrecisions_use_uses_B[0][q] = ReplicateOp.DefAverageLogarithm<Gamma>(nodeNoisePrecisions_rep0_B[q], nodeNoisePrecisions_use_uses_B[0][q]);
				}
				// Message to 'nodeNoisePrecisions_use' from Replicate factor
				this.nodeNoisePrecisions_use_B = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Gamma,double>>(nodeNoisePrecisions_use_uses_B, this.nodeNoisePrecisions_use_B);
				for(int q = 0; q<Q; q++) {
					// Message to 'nodeNoisePrecisions_marginal' from Variable factor
					this.nodeNoisePrecisions_marginal_F[q] = VariableVmpOp.MarginalAverageLogarithm<Gamma>(this.nodeNoisePrecisions_use_B[q], this.NodeNoisePrecisionPrior[q], this.nodeNoisePrecisions_marginal_F[q]);
					for(int n = 0; n<N; n++) {
						// Message to 'nodeFunctionValuesClean_uses' from Gaussian factor
						nodeFunctionValuesClean_uses_B[1][q][n] = GaussianOp.MeanAverageLogarithm(this.nodeFunctionValuesPredictive_marginal_F[q][n], this.nodeNoisePrecisions_marginal_F[q]);
						// Message to 'nodeFunctionValuesClean_uses' from Gaussian factor
						nodeFunctionValuesClean_uses_B[0][q][n] = GaussianOp.MeanAverageLogarithm(this.nodeFunctionValues_marginal_F[q][n], this.nodeNoisePrecisions_marginal_F[q]);
					}
				}
				// Message to 'nodeFunctionValuesClean' from Replicate factor
				nodeFunctionValuesClean_B = ReplicateOp.DefAverageLogarithm<DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]>>(nodeFunctionValuesClean_uses_B, nodeFunctionValuesClean_B);
				for(int q = 0; q<Q; q++) {
					// Message to 'nodeFunctions_use' from ArrayFromVector factor
					nodeFunctions_use_B[q] = ArrayFromVectorOp.VectorAverageLogarithm(nodeFunctionValuesClean_B[q], nodeFunctions_use_B[q]);
					// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
					nodeFunctions_B_q__SampleVariance[q] = VectorGaussianScaledPrecisionOp.SampleVariance(this.nodeFunctions_marginal_F[q], nodeFunctions_B_q__SampleVariance[q]);
					// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
					nodeFunctions_B_q__SampleMean[q] = VectorGaussianScaledPrecisionOp.SampleMean(this.nodeFunctions_marginal_F[q], nodeFunctions_B_q__SampleVariance[q], nodeFunctions_B_q__SampleMean[q]);
					// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
					nodeFunctions_B_q__SampleMean_ESamplePrecisionSample[q] = VectorGaussianScaledPrecisionOp.ESamplePrecisionSample(nodeFunctions_B_q__SampleMean[q], nodeFunctions_B_q__SampleVariance[q], this.k_node_inverse);
					// Message to 'nodeSignalPrecisions_use' from VectorGaussianScaled factor
					nodeSignalPrecisions_use_B[q] = VectorGaussianScaledPrecisionOp.ScalingAverageLogarithm(this.k_node_inverse, nodeFunctions_B_q__SampleMean_ESamplePrecisionSample[q]);
					// Message to 'nodeSignalPrecisions_marginal' from Variable factor
					this.nodeSignalPrecisions_marginal_F[q] = VariableVmpOp.MarginalAverageLogarithm<Gamma>(nodeSignalPrecisions_use_B[q], this.NodeSignalPrecisionsPrior[q], this.nodeSignalPrecisions_marginal_F[q]);
					// Message to 'nodeFunctions' from VectorGaussianScaled factor
					nodeFunctions_F[q] = VectorGaussianScaledPrecisionOp.SampleAverageLogarithm(this.nodeSignalPrecisions_marginal_F[q], this.k_node_inverse, nodeFunctions_F[q]);
					// Message to 'nodeFunctions_marginal' from Variable factor
					this.nodeFunctions_marginal_F[q] = VariableVmpOp.MarginalAverageLogarithm<VectorGaussian>(nodeFunctions_use_B[q], nodeFunctions_F[q], this.nodeFunctions_marginal_F[q]);
					// Message to 'nodeFunctions_use' from Variable factor
					this.nodeFunctions_use_F[q] = Factor.Copy<VectorGaussian>(this.nodeFunctions_marginal_F[q]);
				}
				for(int d = 0; d<D; d++) {
					// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
					meanFunctions_B_d__SampleVariance[d] = GPFactor.SampleVariance(meanFunctions_marginal_F[d], meanFunctions_B_d__SampleVariance[d]);
				}
				this.OnProgressChanged(new ProgressChangedEventArgs(iteration));
			}
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep0_uses_B = default(DistributionStructArray<Bernoulli,bool>[]);
			// Create array for 'ev_selector_cases_0_rep0_uses' Backwards messages.
			ev_selector_cases_0_rep0_uses_B = new DistributionStructArray<Bernoulli,bool>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'ev_selector_cases_0_rep0_uses' Backwards messages.
				ev_selector_cases_0_rep0_uses_B[_ind] = new DistributionStructArray<Bernoulli,bool>(this.q);
				for(int q = 0; q<Q; q++) {
					ev_selector_cases_0_rep0_uses_B[_ind][q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
			}
			for(int q = 0; q<Q; q++) {
				// Message to 'ev_selector_cases_0_rep0_uses' from Random factor
				ev_selector_cases_0_rep0_uses_B[0][q] = Bernoulli.FromLogOdds(UnaryOp<double>.AverageLogFactor<Gamma>(this.nodeNoisePrecisions_marginal_F[q], this.NodeNoisePrecisionPrior[q]));
				// Message to 'ev_selector_cases_0_rep0_uses' from Variable factor
				ev_selector_cases_0_rep0_uses_B[1][q] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<Gamma>(this.nodeNoisePrecisions_marginal_F[q], this.NodeNoisePrecisionPrior[q]));
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep0_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep0' Backwards messages.
			ev_selector_cases_0_rep0_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep0_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases_0_rep0' from Replicate factor
			ev_selector_cases_0_rep0_B = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep0_uses_B, ev_selector_cases_0_rep0_B);
			Bernoulli[] ev_selector_cases_0_uses_B = default(Bernoulli[]);
			// Create array for 'ev_selector_cases_0_uses' Backwards messages.
			ev_selector_cases_0_uses_B = new Bernoulli[69];
			for(int _ind = 0; _ind<69; _ind++) {
				ev_selector_cases_0_uses_B[_ind] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[1] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep0_B, ev_selector_cases_0_uses_B[1]);
			// Message to 'ev_selector_cases_0_uses' from Random factor
			ev_selector_cases_0_uses_B[7] = Bernoulli.FromLogOdds(UnaryOp<double>.AverageLogFactor<Gamma>(this.noisePrecision_marginal_F, this.NoisePrecisionPrior));
			// Message to 'ev_selector_cases_0_uses' from Variable factor
			ev_selector_cases_0_uses_B[8] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<Gamma>(this.noisePrecision_marginal_F, this.NoisePrecisionPrior));
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep2_uses_B = default(DistributionStructArray<Bernoulli,bool>[]);
			// Create array for 'ev_selector_cases_0_rep2_uses' Backwards messages.
			ev_selector_cases_0_rep2_uses_B = new DistributionStructArray<Bernoulli,bool>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'ev_selector_cases_0_rep2_uses' Backwards messages.
				ev_selector_cases_0_rep2_uses_B[_ind] = new DistributionStructArray<Bernoulli,bool>(this.d);
				for(int d = 0; d<D; d++) {
					ev_selector_cases_0_rep2_uses_B[_ind][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
			}
			for(int d = 0; d<D; d++) {
				// Message to 'ev_selector_cases_0_rep2_uses' from GP factor
				ev_selector_cases_0_rep2_uses_B[0][d] = Bernoulli.FromLogOdds(GPFactor.AverageLogFactor(meanFunctions_B_d__SampleMean[d], 1, this.vint__0_Buffer[d]));
				// Message to 'ev_selector_cases_0_rep2_uses' from Variable factor
				ev_selector_cases_0_rep2_uses_B[1][d] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<VectorGaussian>(meanFunctions_marginal_F[d], meanFunctions_F[d]));
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep2_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep2' Backwards messages.
			ev_selector_cases_0_rep2_B = new DistributionStructArray<Bernoulli,bool>(this.d);
			for(int d = 0; d<D; d++) {
				ev_selector_cases_0_rep2_B[d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases_0_rep2' from Replicate factor
			ev_selector_cases_0_rep2_B = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep2_uses_B, ev_selector_cases_0_rep2_B);
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[12] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep2_B, ev_selector_cases_0_uses_B[12]);
			double _hoist1 = default(double);
			_hoist1 = VectorGaussianScaledPrecisionOp.PrecisionMeanLogDet(this.k_weights_inverse);
			// Buffer for VectorGaussianScaledPrecisionOp.SampleMean
			// Create array for replicates of 'weightFunctions_B_d__q__SampleVariance'
			PositiveDefiniteMatrix[][] weightFunctions_B_d__q__SampleVariance = new PositiveDefiniteMatrix[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for replicates of 'weightFunctions_B_d__q__SampleVariance'
				weightFunctions_B_d__q__SampleVariance[q] = new PositiveDefiniteMatrix[this.d];
				for(int d = 0; d<D; d++) {
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					weightFunctions_B_d__q__SampleVariance[q][d] = VectorGaussianScaledPrecisionOp.SampleVarianceInit(weightFunctions_marginal_F[d, q]);
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					weightFunctions_B_d__q__SampleVariance[q][d] = VectorGaussianScaledPrecisionOp.SampleVariance(weightFunctions_marginal_F[d, q], weightFunctions_B_d__q__SampleVariance[q][d]);
				}
			}
			// Buffer for VectorGaussianScaledPrecisionOp.AverageLogFactor
			// Create array for replicates of 'weightFunctions_B_d__q__SampleMean'
			Vector[][] weightFunctions_B_d__q__SampleMean = new Vector[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for replicates of 'weightFunctions_B_d__q__SampleMean'
				weightFunctions_B_d__q__SampleMean[q] = new Vector[this.d];
				for(int d = 0; d<D; d++) {
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					weightFunctions_B_d__q__SampleMean[q][d] = VectorGaussianScaledPrecisionOp.SampleMeanInit(weightFunctions_marginal_F[d, q]);
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					weightFunctions_B_d__q__SampleMean[q][d] = VectorGaussianScaledPrecisionOp.SampleMean(weightFunctions_marginal_F[d, q], weightFunctions_B_d__q__SampleVariance[q][d], weightFunctions_B_d__q__SampleMean[q][d]);
				}
			}
			// Buffer for VectorGaussianScaledPrecisionOp.AverageLogFactor
			// Create array for replicates of 'weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample'
			double[][] weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample = new double[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for replicates of 'weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample'
				weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample[q] = new double[this.d];
				for(int d = 0; d<D; d++) {
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample[q][d] = VectorGaussianScaledPrecisionOp.ESamplePrecisionSample(weightFunctions_B_d__q__SampleMean[q][d], weightFunctions_B_d__q__SampleVariance[q][d], this.k_weights_inverse);
				}
			}
			// Create array for replicates of 'ev_selector_cases_0_rep7_rep0_uses_B'
			DistributionStructArray<Bernoulli,bool>[][] ev_selector_cases_0_rep7_rep0_uses_B = new DistributionStructArray<Bernoulli,bool>[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep7_rep0_uses' Backwards messages.
				ev_selector_cases_0_rep7_rep0_uses_B[q] = new DistributionStructArray<Bernoulli,bool>[2];
				for(int _ind = 0; _ind<2; _ind++) {
					// Create array for 'ev_selector_cases_0_rep7_rep0_uses' Backwards messages.
					ev_selector_cases_0_rep7_rep0_uses_B[q][_ind] = new DistributionStructArray<Bernoulli,bool>(this.d);
					for(int d = 0; d<D; d++) {
						ev_selector_cases_0_rep7_rep0_uses_B[q][_ind][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
					}
				}
				for(int d = 0; d<D; d++) {
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from VectorGaussianScaled factor
					ev_selector_cases_0_rep7_rep0_uses_B[q][0][d] = Bernoulli.FromLogOdds(VectorGaussianScaledPrecisionOp.AverageLogFactor(weightFunctions_B_d__q__SampleMean[q][d], 1, weightFunctions_B_d__q__SampleMean_ESamplePrecisionSample[q][d], _hoist1));
					// Message to 'ev_selector_cases_0_rep7_rep0_uses' from Variable factor
					ev_selector_cases_0_rep7_rep0_uses_B[q][1][d] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<VectorGaussian>(weightFunctions_marginal_F[d, q], _hoist2));
				}
			}

            this.weightKernelOptimiser.Optimise((prec, gradK, gradientVector) =>
                KernelOptimiser.mlfHelperWeight(prec, gradK, gradientVector, weightFunctions_B_d__q__SampleMean,
                weightFunctions_B_d__q__SampleVariance), ref this.k_weights_inverse);

			// Create array for replicates of 'ev_selector_cases_0_rep7_rep0_B'
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep7_rep0_B = new DistributionStructArray<Bernoulli,bool>[this.q];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep7_rep0' Backwards messages.
				ev_selector_cases_0_rep7_rep0_B[q] = new DistributionStructArray<Bernoulli,bool>(this.d);
				for(int d = 0; d<D; d++) {
					ev_selector_cases_0_rep7_rep0_B[q][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
				// Message to 'ev_selector_cases_0_rep7_rep0' from Replicate factor
				ev_selector_cases_0_rep7_rep0_B[q] = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep7_rep0_uses_B[q], ev_selector_cases_0_rep7_rep0_B[q]);
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep7_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep7' Backwards messages.
			ev_selector_cases_0_rep7_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep7_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				// Message to 'ev_selector_cases_0_rep7' from Replicate factor
				ev_selector_cases_0_rep7_B[q] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep7_rep0_B[q], ev_selector_cases_0_rep7_B[q]);
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[24] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep7_B, ev_selector_cases_0_uses_B[24]);
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep11_uses_B = default(DistributionStructArray<Bernoulli,bool>[]);
			// Create array for 'ev_selector_cases_0_rep11_uses' Backwards messages.
			ev_selector_cases_0_rep11_uses_B = new DistributionStructArray<Bernoulli,bool>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'ev_selector_cases_0_rep11_uses' Backwards messages.
				ev_selector_cases_0_rep11_uses_B[_ind] = new DistributionStructArray<Bernoulli,bool>(this.q);
				for(int q = 0; q<Q; q++) {
					ev_selector_cases_0_rep11_uses_B[_ind][q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
			}
			for(int q = 0; q<Q; q++) {
				// Message to 'ev_selector_cases_0_rep11_uses' from Random factor
				ev_selector_cases_0_rep11_uses_B[0][q] = Bernoulli.FromLogOdds(UnaryOp<double>.AverageLogFactor<Gamma>(this.nodeSignalPrecisions_marginal_F[q], this.NodeSignalPrecisionsPrior[q]));
				// Message to 'ev_selector_cases_0_rep11_uses' from Variable factor
				ev_selector_cases_0_rep11_uses_B[1][q] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<Gamma>(this.nodeSignalPrecisions_marginal_F[q], this.NodeSignalPrecisionsPrior[q]));
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep11_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep11' Backwards messages.
			ev_selector_cases_0_rep11_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep11_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases_0_rep11' from Replicate factor
			ev_selector_cases_0_rep11_B = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep11_uses_B, ev_selector_cases_0_rep11_B);
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[36] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep11_B, ev_selector_cases_0_uses_B[36]);
			double _hoist4 = default(double);
			_hoist4 = VectorGaussianScaledPrecisionOp.PrecisionMeanLogDet(this.k_node_inverse);
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep12_uses_B = default(DistributionStructArray<Bernoulli,bool>[]);
			// Create array for 'ev_selector_cases_0_rep12_uses' Backwards messages.
			ev_selector_cases_0_rep12_uses_B = new DistributionStructArray<Bernoulli,bool>[2];
			for(int _ind = 0; _ind<2; _ind++) {
				// Create array for 'ev_selector_cases_0_rep12_uses' Backwards messages.
				ev_selector_cases_0_rep12_uses_B[_ind] = new DistributionStructArray<Bernoulli,bool>(this.q);
				for(int q = 0; q<Q; q++) {
					ev_selector_cases_0_rep12_uses_B[_ind][q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
			}
			for(int q = 0; q<Q; q++) {
				// Message to 'ev_selector_cases_0_rep12_uses' from VectorGaussianScaled factor
				ev_selector_cases_0_rep12_uses_B[0][q] = Bernoulli.FromLogOdds(VectorGaussianScaledPrecisionOp.AverageLogFactor(nodeFunctions_B_q__SampleMean[q], this.nodeSignalPrecisions_marginal_F[q], nodeFunctions_B_q__SampleMean_ESamplePrecisionSample[q], _hoist4));
				// Message to 'ev_selector_cases_0_rep12_uses' from Variable factor
				ev_selector_cases_0_rep12_uses_B[1][q] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<VectorGaussian>(this.nodeFunctions_marginal_F[q], nodeFunctions_F[q]));
			}

            this.nodeKernelOptimiser.Optimise((prec, gradK, gradientVector) =>
                KernelOptimiser.mlfHelperNode(prec, gradK, gradientVector, nodeFunctions_B_q__SampleMean,
                nodeFunctions_B_q__SampleVariance, this.nodeSignalPrecisions_marginal_F), ref this.k_node_inverse);

			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep12_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep12' Backwards messages.
			ev_selector_cases_0_rep12_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep12_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases_0_rep12' from Replicate factor
			ev_selector_cases_0_rep12_B = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep12_uses_B, ev_selector_cases_0_rep12_B);
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[39] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep12_B, ev_selector_cases_0_uses_B[39]);
			// Create array for replicates of 'ev_selector_cases_0_rep14_rep0_uses_B'
			DistributionStructArray<Bernoulli,bool>[][] ev_selector_cases_0_rep14_rep0_uses_B = new DistributionStructArray<Bernoulli,bool>[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep14_rep0_uses' Backwards messages.
				ev_selector_cases_0_rep14_rep0_uses_B[q] = new DistributionStructArray<Bernoulli,bool>[2];
				for(int _ind = 0; _ind<2; _ind++) {
					// Create array for 'ev_selector_cases_0_rep14_rep0_uses' Backwards messages.
					ev_selector_cases_0_rep14_rep0_uses_B[q][_ind] = new DistributionStructArray<Bernoulli,bool>(this.n);
					for(int n = 0; n<N; n++) {
						ev_selector_cases_0_rep14_rep0_uses_B[q][_ind][n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
					}
				}
				for(int n = 0; n<N; n++) {
					// Message to 'ev_selector_cases_0_rep14_rep0_uses' from Gaussian factor
					ev_selector_cases_0_rep14_rep0_uses_B[q][0][n] = Bernoulli.FromLogOdds(GaussianOp.AverageLogFactor(this.nodeFunctionValues_marginal_F[q][n], nodeFunctionValuesClean_F[q][n], this.nodeNoisePrecisions_marginal_F[q]));
					// Message to 'ev_selector_cases_0_rep14_rep0_uses' from Variable factor
					ev_selector_cases_0_rep14_rep0_uses_B[q][1][n] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<Gaussian>(this.nodeFunctionValues_marginal_F[q][n], nodeFunctionValues_F[q][n]));
				}
			}
			// Create array for replicates of 'ev_selector_cases_0_rep14_rep0_B'
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep14_rep0_B = new DistributionStructArray<Bernoulli,bool>[this.q];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep14_rep0' Backwards messages.
				ev_selector_cases_0_rep14_rep0_B[q] = new DistributionStructArray<Bernoulli,bool>(this.n);
				for(int n = 0; n<N; n++) {
					ev_selector_cases_0_rep14_rep0_B[q][n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
				// Message to 'ev_selector_cases_0_rep14_rep0' from Replicate factor
				ev_selector_cases_0_rep14_rep0_B[q] = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep14_rep0_uses_B[q], ev_selector_cases_0_rep14_rep0_B[q]);
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep14_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep14' Backwards messages.
			ev_selector_cases_0_rep14_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep14_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				// Message to 'ev_selector_cases_0_rep14' from Replicate factor
				ev_selector_cases_0_rep14_B[q] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep14_rep0_B[q], ev_selector_cases_0_rep14_B[q]);
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[43] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep14_B, ev_selector_cases_0_uses_B[43]);
			// Create array for replicates of 'ev_selector_cases_0_rep21_rep0_uses_B'
			DistributionStructArray<Bernoulli,bool>[][] ev_selector_cases_0_rep21_rep0_uses_B = new DistributionStructArray<Bernoulli,bool>[this.q][];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep21_rep0_uses' Backwards messages.
				ev_selector_cases_0_rep21_rep0_uses_B[q] = new DistributionStructArray<Bernoulli,bool>[2];
				for(int _ind = 0; _ind<2; _ind++) {
					// Create array for 'ev_selector_cases_0_rep21_rep0_uses' Backwards messages.
					ev_selector_cases_0_rep21_rep0_uses_B[q][_ind] = new DistributionStructArray<Bernoulli,bool>(this.n);
					for(int n = 0; n<N; n++) {
						ev_selector_cases_0_rep21_rep0_uses_B[q][_ind][n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
					}
				}
				for(int n = 0; n<N; n++) {
					// Message to 'ev_selector_cases_0_rep21_rep0_uses' from Gaussian factor
					ev_selector_cases_0_rep21_rep0_uses_B[q][0][n] = Bernoulli.FromLogOdds(GaussianOp.AverageLogFactor(this.nodeFunctionValuesPredictive_marginal_F[q][n], nodeFunctionValuesClean_F[q][n], this.nodeNoisePrecisions_marginal_F[q]));
					// Message to 'ev_selector_cases_0_rep21_rep0_uses' from Variable factor
					ev_selector_cases_0_rep21_rep0_uses_B[q][1][n] = Bernoulli.FromLogOdds(VariableVmpOp.AverageLogFactor<Gaussian>(this.nodeFunctionValuesPredictive_marginal_F[q][n], nodeFunctionValuesPredictive_F[q][n]));
				}
			}
			// Create array for replicates of 'ev_selector_cases_0_rep21_rep0_B'
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep21_rep0_B = new DistributionStructArray<Bernoulli,bool>[this.q];
			for(int q = 0; q<Q; q++) {
				// Create array for 'ev_selector_cases_0_rep21_rep0' Backwards messages.
				ev_selector_cases_0_rep21_rep0_B[q] = new DistributionStructArray<Bernoulli,bool>(this.n);
				for(int n = 0; n<N; n++) {
					ev_selector_cases_0_rep21_rep0_B[q][n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
				// Message to 'ev_selector_cases_0_rep21_rep0' from Replicate factor
				ev_selector_cases_0_rep21_rep0_B[q] = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep21_rep0_uses_B[q], ev_selector_cases_0_rep21_rep0_B[q]);
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep21_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep21' Backwards messages.
			ev_selector_cases_0_rep21_B = new DistributionStructArray<Bernoulli,bool>(this.q);
			for(int q = 0; q<Q; q++) {
				ev_selector_cases_0_rep21_B[q] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				// Message to 'ev_selector_cases_0_rep21' from Replicate factor
				ev_selector_cases_0_rep21_B[q] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep21_rep0_B[q], ev_selector_cases_0_rep21_B[q]);
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[55] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep21_B, ev_selector_cases_0_uses_B[55]);
			// Create array for replicates of 'ev_selector_cases_0_rep25_rep0_uses_B'
			DistributionStructArray<Bernoulli,bool>[][] ev_selector_cases_0_rep25_rep0_uses_B = new DistributionStructArray<Bernoulli,bool>[this.n][];
			for(int n = 0; n<N; n++) {
				// Create array for 'ev_selector_cases_0_rep25_rep0_uses' Backwards messages.
				ev_selector_cases_0_rep25_rep0_uses_B[n] = new DistributionStructArray<Bernoulli,bool>[5];
				for(int _ind = 0; _ind<5; _ind++) {
					// Create array for 'ev_selector_cases_0_rep25_rep0_uses' Backwards messages.
					ev_selector_cases_0_rep25_rep0_uses_B[n][_ind] = new DistributionStructArray<Bernoulli,bool>(this.d);
					for(int d = 0; d<D; d++) {
						ev_selector_cases_0_rep25_rep0_uses_B[n][_ind][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
					}
				}
				for(int d = 0; d<D; d++) {
					if (!this.IsMissing[d, n]) {
						// Message to 'ev_selector_cases_0_rep25_rep0_uses' from Gaussian factor
						ev_selector_cases_0_rep25_rep0_uses_B[n][4][d] = Bernoulli.FromLogOdds(GaussianOp.AverageLogFactor(this.ObservedData[d, n], this.noiseLessY_F[d, n], this.noisePrecision_marginal_F));
					}
				}
			}
			// Create array for replicates of 'ev_selector_cases_0_rep25_rep0_B'
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep25_rep0_B = new DistributionStructArray<Bernoulli,bool>[this.n];
			for(int n = 0; n<N; n++) {
				// Create array for 'ev_selector_cases_0_rep25_rep0' Backwards messages.
				ev_selector_cases_0_rep25_rep0_B[n] = new DistributionStructArray<Bernoulli,bool>(this.d);
				for(int d = 0; d<D; d++) {
					ev_selector_cases_0_rep25_rep0_B[n][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				}
				// Message to 'ev_selector_cases_0_rep25_rep0' from Replicate factor
				ev_selector_cases_0_rep25_rep0_B[n] = ReplicateOp.DefAverageLogarithm<DistributionStructArray<Bernoulli,bool>>(ev_selector_cases_0_rep25_rep0_uses_B[n], ev_selector_cases_0_rep25_rep0_B[n]);
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep25_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep25' Backwards messages.
			ev_selector_cases_0_rep25_B = new DistributionStructArray<Bernoulli,bool>(this.n);
			for(int n = 0; n<N; n++) {
				ev_selector_cases_0_rep25_B[n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				// Message to 'ev_selector_cases_0_rep25' from Replicate factor
				ev_selector_cases_0_rep25_B[n] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep25_rep0_B[n], ev_selector_cases_0_rep25_B[n]);
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[64] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep25_B, ev_selector_cases_0_uses_B[64]);
			// Create array for replicates of 'ev_selector_cases_0_rep27_rep0_B'
			DistributionStructArray<Bernoulli,bool>[] ev_selector_cases_0_rep27_rep0_B = new DistributionStructArray<Bernoulli,bool>[this.n];
			for(int n = 0; n<N; n++) {
				// Create array for 'ev_selector_cases_0_rep27_rep0' Backwards messages.
				ev_selector_cases_0_rep27_rep0_B[n] = new DistributionStructArray<Bernoulli,bool>(this.d);
				for(int d = 0; d<D; d++) {
					ev_selector_cases_0_rep27_rep0_B[n][d] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
					if (this.IsMissing[d, n]) {
						// Message to 'ev_selector_cases_0_rep27_rep0' from Gaussian factor
						ev_selector_cases_0_rep27_rep0_B[n][d] = Bernoulli.FromLogOdds(GaussianOp.AverageLogFactor(this.ObservedData[d, n], 0, 1));
					}
				}
			}
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_0_rep27_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases_0_rep27' Backwards messages.
			ev_selector_cases_0_rep27_B = new DistributionStructArray<Bernoulli,bool>(this.n);
			for(int n = 0; n<N; n++) {
				ev_selector_cases_0_rep27_B[n] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
				// Message to 'ev_selector_cases_0_rep27' from Replicate factor
				ev_selector_cases_0_rep27_B[n] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep27_rep0_B[n], ev_selector_cases_0_rep27_B[n]);
			}
			// Message to 'ev_selector_cases_0_uses' from Replicate factor
			ev_selector_cases_0_uses_B[68] = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_rep27_B, ev_selector_cases_0_uses_B[68]);
			Bernoulli ev_selector_cases_0_B = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			// Message to 'ev_selector_cases_0' from Replicate factor
			ev_selector_cases_0_B = ReplicateOp.DefAverageLogarithm<Bernoulli>(ev_selector_cases_0_uses_B, ev_selector_cases_0_B);
			DistributionStructArray<Bernoulli,bool> ev_selector_cases_B = default(DistributionStructArray<Bernoulli,bool>);
			// Create array for 'ev_selector_cases' Backwards messages.
			ev_selector_cases_B = new DistributionStructArray<Bernoulli,bool>(2);
			for(int _ind0 = 0; _ind0<2; _ind0++) {
				ev_selector_cases_B[_ind0] = ArrayHelper.MakeUniform<Bernoulli>(new Bernoulli());
			}
			// Message to 'ev_selector_cases' from Copy factor
			ev_selector_cases_B[0] = Factor.Copy<Bernoulli>(ev_selector_cases_0_B);
			Bernoulli ev_selector_B = ArrayHelper.MakeUniform<Bernoulli>(vBernoulli0);
			// Message to 'ev_selector' from Cases factor
			ev_selector_B = CasesOp.BAverageLogarithm(ev_selector_cases_B);
			// Message to 'ev_marginal' from Variable factor
			this.ev_marginal_F = VariableVmpOp.MarginalAverageLogarithm<Bernoulli>(ev_selector_B, vBernoulli0, this.ev_marginal_F);
			// Create array for 'weightFunctionValuesPredictive' Forwards messages.
			this.weightFunctionValuesPredictive_F = new DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]>(this.d, this.q);
			for(int d = 0; d<D; d++) {
				for(int q = 0; q<Q; q++) {
					// Create array for 'weightFunctionValuesPredictive' Forwards messages.
					this.weightFunctionValuesPredictive_F[d, q] = new DistributionStructArray<Gaussian,double>(this.n);
					for(int _iv2 = 0; _iv2<N; _iv2++) {
						this.weightFunctionValuesPredictive_F[d, q][_iv2] = ArrayHelper.MakeUniform<Gaussian>(new Gaussian());
					}
					// Message to 'weightFunctionValuesPredictive' from ArrayFromVector factor
					this.weightFunctionValuesPredictive_F[d, q] = ArrayFromVectorOp.ArrayAverageLogarithm<DistributionStructArray<Gaussian,double>>(weightFunctions_marginal_F[d, q], this.weightFunctionValuesPredictive_F[d, q]);
				}
			}
			this.Changed_numberOfIterationsDecreased_Q_N_D_isMissing_observedData_noisePrecisionPrior_nodeNoisePrecis0_iterationsDone = numberOfIterations;
		}

		/// <summary>
		/// Returns the marginal distribution for 'ev' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public Bernoulli EvMarginal()
		{
			return this.ev_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'nodeNoisePrecisions' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionStructArray<Gamma,double> NodeNoisePrecisionsMarginal()
		{
			return this.nodeNoisePrecisions_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'noisePrecision' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public Gamma NoisePrecisionMarginal()
		{
			return this.noisePrecision_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'meanFunctionValues' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> MeanFunctionValuesMarginal()
		{
			return this.meanFunctionValues_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'weightFunctionValues' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]> WeightFunctionValuesMarginal()
		{
			return this.weightFunctionValues_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'nodeSignalPrecisions' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionStructArray<Gamma,double> NodeSignalPrecisionsMarginal()
		{
			return this.nodeSignalPrecisions_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'nodeFunctionValues' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> NodeFunctionValuesMarginal()
		{
			return this.nodeFunctionValues_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'noiseLessY' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionStructArray2D<Gaussian,double> NoiseLessYMarginal()
		{
			return this.noiseLessY_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'nodeFunctionValuesPredictive' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionRefArray<DistributionStructArray<Gaussian,double>,double[]> NodeFunctionValuesPredictiveMarginal()
		{
			return this.nodeFunctionValuesPredictive_marginal_F;
		}

		/// <summary>
		/// Returns the marginal distribution for 'weightFunctionValuesPredictive' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionRefArray2D<DistributionStructArray<Gaussian,double>,double[]> WeightFunctionValuesPredictiveMarginal()
		{
			return this.weightFunctionValuesPredictive_F;
		}

		#endregion

		#region Events
		/// <summary>Event that is fired when the progress of inference changes, typically at the end of one iteration of the inference algorithm.</summary>
		public event EventHandler<ProgressChangedEventArgs> ProgressChanged;
		#endregion

	}

}
