...
namespace MicrosoftResearch.Infer.Distributions.Kernels
{
    ...
    /// </summary>
    [Serializable]
    public class MixtureKernel : KernelFunction
    {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < kernels.Count; i++)
            {
                ...
                sb.Append(weights[i]+"*"+kernels[i].ToString());
            }
            return sb.ToString();
        }
        ...
        public override double EvaluateX1X2(Vector x1, Vector x2, ref Vector x1Deriv, ref Vector logThetaDeriv)
        {
            ...
            for (int kx = 0; kx < kernels.Count; kx++)
            {
                ...
                if (((object)logThetaDeriv) != null)
                    ...
                    lthd = null;
                result += weights[kx]*k.EvaluateX1X2(x1, x2, ref lx1d, ref lthd);
                if (((object)lthd) != null)
                {
                    for (int ix = 0; ix < thcnt; ix++)
                    {
                        logThetaDeriv[derivX++] = weights[kx] * lthd[ix];
                    }
                }
                if (((object)lx1d) != null)
                {
                    x1Deriv.SetToSum(x1Deriv, weights[kx] * lx1d);
                }
            }
            return result;
        }
        ...
        public override double EvaluateX(Vector x, ref Vector xDeriv, ref Vector logThetaDeriv)
        {
            ...
            for (int kx = 0; kx < kernels.Count; kx++)
            {
                ...
                if (((object)logThetaDeriv) != null)
                {
                    ...
                    lthd = null;
                result += weights[kx] * k.EvaluateX(x, ref lx1d, ref lthd);
                if (((object)lthd) != null)
                {
                    for (int ix = 0; ix < thcnt; ix++)
                    {
                ...
                if (((object)lx1d) != null)
                {
                    xDeriv.SetToSum(xDeriv, weights[kx] * lx1d);
                }
            }
            return result;
        }
